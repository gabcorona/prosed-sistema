// prosed-form-logic.js
// ── DATA ──────────────────────────────────────────────────────────────────
const CID = new URLSearchParams(window.location.search).get('c');
let contest=null, registrations=[], coupons=[], cfg={};

function loadData(){
  try{
    const c=localStorage.getItem('p_c'),r=localStorage.getItem('p_r'),
          cp=localStorage.getItem('p_cp'),cf=localStorage.getItem('p_cfg');
    if(c){const all=JSON.parse(c);contest=all.find(x=>x.id===CID)||null;}
    if(r)registrations=JSON.parse(r);
    if(cp)coupons=JSON.parse(cp);
    if(cf)cfg=JSON.parse(cf);
  }catch(e){}
}
const PROXY=()=>cfg.proxyUrl||'http://localhost:3001';
...
function render(){
  ...
  [rDados,rToxico,rPacote,rAgenda,rDocs,rCheckout][step]?.();
}
...
function updateNav(){
  ...
  document.querySelectorAll('.step-btn').forEach((b,i)=>{b.classList.remove('active','done');if(i===step)b.classList.add('active');else if(i<step)b.classList.add('done');});
}
function banner(){
  return `<div class="contest-banner fade"><div class="contest-banner-name">📋 ${contest.nome}</div>
  <div class="contest-banner-meta">${contest.orgao}${contest.prazo?' · Prazo: '+contest.prazo:''}</div>
  ${contest.resumo?`<div class="contest-resumo">${contest.resumo}</div>`:''}</div>`;
}
...
function rDados(){
  ...
  <div class="s-card fade"><div class="s-title"><span class="s-num">1</span> Dados Pessoais</div>
    <div class="field" id="f-nome"><label class="fl">Nome Completo <span class="req">*</span></label>
      <input id="nome" type="text" placeholder="Nome e sobrenome" value="${h(fd.nome)}"/></div>
    <div class="g2">
      ...
      <div class="field" id="f-matricula"><label class="fl">Nº de Inscrição <span class="req">*</span></label>
        <input id="matricula" type="tel" class="mono" placeholder="20250001" value="${h(fd.matricula)}" oninput="this.value=this.value.replace(/\\D/g,'').slice(0,12)"/></div>
    </div>
    <div class="g2">
      ...
      <div class="field" id="f-orgao"><label class="fl">Órgão Expedidor <span class="req">*</span></label>
        <input id="orgao" type="text" placeholder="SSP" value="${h(fd.orgao)}"/></div>
    </div>
    <div class="g2">
...
function rPacote(){
  ...
  set('main',`<div class="s-card fade"><div class="s-title"><span class="s-num">3</span> Grupo de Indicação e Pacote</div>
    ...
    <div class="field" id="f-pacote"><label class="fl">Tipo de Pacote <span class="req">*</span></label>${pkgH}${avH}</div>
...
function rDates(city){
  set('date-grid',getDates(city).map(date=>{
    ...
    const full=avail<=0,parts=date.split('/');
...
function rTimes(city,date){
  set('time-grid',getSlots(city,date).map(s=>{
    const avail=s.max-s.booked,full=avail<=0;
  ...
  set('main',`<div class="s-card fade"><div class="s-title"><span class="s-num">5</span> Comprovante de Inscrição</div>
    <div class="field" id="f-file">
      <div class="upload-zone ${selFile?'has-file':''}" onclick="document.getElementById('fi').click()">
        ...
        <div style="font-size:.74rem;color:var(--white-dim);margin-top:4px">${selFile?(selFile.size/1024/1024).toFixed(2)+' MB — Toque para trocar':'PDF, JPG ou PNG · Máx. 10MB'}</div>
      </div>
...
function hFile(inp){
  ...
  if(!['application/pdf','image/jpeg','image/png'].includes(file.type)){showE('f-file','Formato inválido. Use PDF, JPG ou PNG');return;}
  ...
  set('main',`<div class="checkout-section fade">
    ...
    ${total>0?`<div class="s-card">
      ...
      <div id="pix-sec" style="display:${payMethod==='pix'?'':'none'}">
        <div id="pix-pend">
          <div style="font-size:.84rem;color:var(--white-dim);margin-bottom:14px;line-height:1.6">Clique em <strong>"Gerar PIX"</strong> para criar a cobrança e exibir o QR Code.</div>
    ...
    </div>`:`<div class="s-card" style="text-align:center;padding:20px"><div style="font-size:1.5rem;margin-bottom:8px">🎉</div>
      <div style="font-weight:700;font-size:1rem">Total: R$ 0,00 — Desconto total aplicado!</div></div>`}
...
function sPay(m,el){
  ...
  const {total}=calcT();
  if(total>0&&m!=='pix'&&!btn){
    const btns=document.getElementById('co-btns');
    const nb=document.createElement('button');nb.className='btn-primary';nb.id='pay-btn';nb.onclick=subCard;
    nb.innerHTML=`<span id="pay-txt">${m==='credit'?'💳 Pagar com Crédito':'🏧 Pagar com Débito'}</span>`;
    btns.appendChild(nb);
  }
}
...
async function subCard(){
  ...
  if(!num||!name||!exp||!cvv||!cep||!nend){toast('Preencha todos os dados do cartão.','err');return;}
  ...
  try{
    ...
    const pr=await pp(ep,{customerId:cr.customerId,value:total,description:`PROSED – ${contest.nome} – ${fd.nome}`,installmentCount:installments,
    ...
    finalize(pr.paymentId,'paid');
  }catch(e){
    toast('Erro no pagamento: '+e.message,'err');
...
async function gPIX(){
  ...
  try{
    ...
    toast('Erro ao gerar PIX: '+e.message,'err');
...
function startPoll(pid){
  ...
  pixPollTimer=setInterval(async()=>{
    try{
      ...
      if(d.status==='RECEIVED'||d.status==='CONFIRMED'){clearInterval(pixPollTimer);finalize(pid,'paid');}
    }catch(e){}
  },4000);
}
function cpPix(){const t=document.getElementById('pix-ce')?.textContent||'';navigator.clipboard.writeText(t).then(()=>toast('Chave PIX copiada!','ok'));}
...
// ── FINALIZE ──────────────────────────────────────────────────────────────
function finZero(){finalize('','paid');}
function finalize(asaasId,payStatus){
  if(pixPollTimer)clearInterval(pixPollTimer);
  const slot=(contest.slots||[]).find(s=>s.id===selSlotId);if(slot)slot.booked++;
  if(appliedCoupon)appliedCoupon.uses++;
  const{total,disc}=calcT();
  const exNoms=selExames.map(id=>(contest.exames||[]).find(x=>x.id===id)?.nome||id);
  const id='PRSD-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6).toUpperCase();
  const rec={id,contestId:contest.id,submittedAt:new Date().toLocaleString('pt-BR'),
    nome:fd.nome,cpf:fd.cpf,matricula:fd.matricula,rg:fd.rg,orgao:fd.orgao,uf:fd.uf,
    nasc:fd.nasc,sexo:fd.sexo,cel:fd.cel,email:fd.email,
    trat:fd.trat,psico:fd.psico,med:fd.med||'',coleta:fd.coleta,
    grupo:fd.grupo==='furlani'?'Grupo Furlani + Geral':'Grupo Geral',
    pacoteLabel:fd.pacoteLabel,pacoteId:fd.pacoteId,examesSel:exNoms,
    slotId:selSlotId,slotCity:slot?.city||'',slotDate:slot?.date||'',slotTime:slot?.time||'',
  ...
  registrations.unshift(rec);lastReg=rec;
  try{
    localStorage.setItem('p_r',JSON.stringify(registrations));
    const allC=JSON.parse(localStorage.getItem('p_c')||'[]');
    const idx=allC.findIndex(c=>c.id===contest.id);
    if(idx>=0){allC[idx].slots=contest.slots;localStorage.setItem('p_c',JSON.stringify(allC));}
    if(appliedCoupon){const cp=JSON.parse(localStorage.getItem('p_cp')||'[]');const ci=cp.findIndex(x=>x.id===appliedCoupon.id);if(ci>=0){cp[ci].uses++;localStorage.setItem('p_cp',JSON.stringify(cp));}}
  }catch(e){}
  rSuccess(rec);
}
...
function bldPrint(r){
  ...
  <div class="ps"><div class="ps-title">Dados Pessoais</div>
    ...
    <div class="pr"><span class="pk">RG:</span><span>${r.rg} / ${r.orgao}-${r.uf}</span></div>
  ...
  <div class="ps"><div class="ps-title">Pagamento</div>
    <div class="pr"><span class="pk">Subtotal:</span><span>R$ ${brl((r.total||0)+(r.discount||0))}</span></div>
    ${r.discount?`<div class="pr"><span class="pk">Desconto (${r.cupom}):</span><span>– R$ ${brl(r.discount)}</span></div>`:''}
...
// ── NAVIGATION ────────────────────────────────────────────────────────────
function next(){if(!val(step))return;save(step);step=Math.min(step+1,5);render();window.scrollTo({top:0,behavior:'smooth'});}
function prev(){save(step);step=Math.max(step-1,0);render();window.scrollTo({top:0,behavior:'smooth'});}
function save(s){
  const g=id=>{const el=document.getElementById(id);return el?el.value:'';}
  if(s===0)Object.assign(fd,{nome:g('nome'),cpf:g('cpf'),matricula:g('matricula'),rg:g('rg'),orgao:g('orgao'),uf:g('uf'),nasc:g('nasc'),sexo:g('sexo'),cel:g('cel'),email:g('email')});
...
function val(s){
  ...
  if(s===0){
    ...
    else if(registrations.some(r=>r.cpf===cpf&&r.contestId===CID&&r.status!=='cancelado')){showE('f-cpf','CPF já cadastrado neste concurso');ok=false;}
    if(!document.getElementById('matricula')?.value.trim()){showE('f-matricula','Campo obrigatório');ok=false;}
    if(!document.getElementById('rg')?.value.trim()){showE('f-rg','Campo obrigatório');ok=false;}
    if(!document.getElementById('orgao')?.value.trim()){showE('f-orgao','Campo obrigatório');ok=false;}
  ...
  if(s===1){let ok=true;
    ...
    return ok;}
  if(s===2){if(!fd.pacoteId){showE('f-pacote','Selecione um pacote');return false;}if(fd.pacoteId==='avulsos'&&selExames.length===0){toast('Selecione pelo menos 1 exame avulso.','err');return false;}if(!document.getElementById('grupo')?.value){showE('f-grupo','Selecione o grupo');return false;}return true;}
  if(s===3){if(!selSlotId){showE('f-slot','Selecione unidade → data → horário');return false;}return true;}
  if(s===4){if(!selFile){showE('f-file','Comprovante obrigatório');return false;}return true;}
  return true;
}
...
function apCup(){
  ...
  if(!code){toast('Digite um código de cupom.','err');return;}
  const cp=coupons.find(c=>c.code===code&&c.active);
  if(!cp){toast('Cupom inválido ou expirado.','err');return;}
  if(cp.maxUses>0&&cp.uses>=cp.maxUses){toast('Cupom esgotado.','err');return;}
  if(cp.validity){const[d,m,y]=cp.validity.split('/');if(new Date()>new Date(y,m-1,d)){toast('Cupom vencido.','err');return;}}
  if(cp.contestId&&cp.contestId!==CID){toast('Cupom não válido para este concurso.','err');return;}
  appliedCoupon=cp;toast('Cupom aplicado!','ok');rCheckout();
}
...
function tMed(){const v=document.getElementById('psico')?.value;const f=document.getElementById('f-med');if(f)f.style.display=v==='Sim'?'':'none';if(v!=='Sim'){const m=document.getElementById('med');if(m)m.value='';}}
...
function toast(msg,type='ok'){const t=document.getElementById('toast');t.className='toast toast-'+type;document.getElementById('toast-icon').textContent=type==='ok'?'✓':'✕';document.getElementById('toast-msg').textContent=msg;t.style.display='flex';setTimeout(()=>t.style.display='none',4200);}
